...
import com.grandfatherpikhto.lessonblegatt01.databinding.FragmentGattBinding
import com.grandfatherpikhto.lessonblegatt01.ui.adapters.RvServicesAdapter
 ...
 */
class GattFragment : Fragment() {

    private var _binding: FragmentGattBinding? = null
    ...
    private val servicesMenuProvider = object : MenuProvider {
        override fun onCreateMenu(menu: Menu, menuInflater: MenuInflater) {
            menuInflater.inflate(R.menu.menu_gatt, menu)
                    ...
                    bleManager.flowConnectionState.collect { state ->
                        when(state) {
                            BleGattManager.State.Connected -> {
                                ...
                                actionConnect.setIcon(R.drawable.ic_bluetooth_disconnect)
                            }
                            BleGattManager.State.Disconnected -> {
                                actionConnect.isEnabled = true
                                actionConnect.title = getString(R.string.device_connect)
                                actionConnect.setIcon(R.drawable.ic_bluetooth_connect)
                            }
                            else -> {
        ...
        override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
            ...
            return when(menuItem.itemId) {
                ...
                R.id.action_scanner -> {
                    bleManager.close()
                    findNavController().navigate(R.id.action_gattFragment_to_scanFragment)
                    true
                }
                else -> {
                    true
    ...
    override fun onCreateView(
        ...
        _bleManager = (requireContext().applicationContext as GattApplication).bleManager
        _binding = FragmentGattBinding.inflate(inflater, container, false)
